...
func (l DefaultLogger) Dump(logData []*model.LogTable, primaryFields []string) ([]map[string]interface{}, error) {
	...
	for _, item := range logData {
		...
		if len(senseFields) == 0 || senseFields[0] == "" {
			for _, item := range l.sourceDb.ListTableField(item.TableName) {
...
func (l DefaultLogger) GetLogger(tableName string, primaryID string, startTime, endTime *time.Time, page, pageSize int) ([]map[string]interface{}, error) {
	res, err := model.LogRepoV2.SearchRecordByField(tableName, primaryID, startTime, endTime, page, pageSize)
	if err != nil {
		return nil, err
	}
...
func (l DefaultLogger) GetAllLogger(tableName string, primaryFields []string, start, end *time.Time, page, pageSize int) ([]map[string]interface{}, error) {
	res, err := model.LogRepoV2.SearchAllRecord(tableName, primaryFields, start, end, page, pageSize)
	if err != nil {
		return nil, err
	}
	result := []map[string]interface{}{}
...
func (l DefaultLogger) TransField(tableName string, records []map[string]interface{}) []map[string]interface{} {
	...
	allfields := l.sourceDb.ListTableField(tableName)
	fieldMapping := map[string]string{}
	for _, item := range allfields {
		fieldMapping[item.FieldID] = item.FieldName
	}
	fieldMappingCache := model.FieldMappingRepo.ListAllFieldMapping(tableName)
	for key, value := range fieldMapping { // 将存在的字段做更新
		fieldMappingCache[key] = value
	}
	model.FieldMappingRepo.UpdateAllFieldByMapping(tableName, fieldMappingCache)
