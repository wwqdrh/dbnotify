...
var logdb *driver.LevelDBDriver
...
// 记录dml 写到磁盘中
func (l LocalLog2) Write(db *driver.LevelDBDriver, tableName string, primaryFields []string, data []map[string]interface{}, outdate, minLog int) error {
	if len(data) == 0 {
		return nil
	}
	logdb = db
	dbName := tableName + "_log.db" // 一般都是同一个数据表的数据
	_, err := logdb.GetDB(dbName)
	if err != nil {
		return err
	}
...
func (l LocalLog2) AddLog(db string, date, field string, record map[string]interface{}, outdate, minLog int) error {
	...
	err := logdb.WriteByArray(db, key, record)
	if err != nil {
		return err
	}
...
func (l LocalLog2) addPin(dbName, date, field string) error {
	...
	_, err := logdb.Get(dbName, key)
	if err != nil {
		if err2 := logdb.Put(dbName, key, []byte("")); err2 != nil {
			return err2
		}
	}
	_, err = logdb.Get(dbName, lastKey)
	if err != nil {
		if err2 := logdb.Put(dbName, lastKey, []byte("")); err2 != nil {
			return err2
		}
	}
...
func (l LocalLog2) removeRecord(dbName, date, field string, day int, minNum int) error {
	...
	nums, err := logdb.GetRangeNum(dbName, start, end)
	...
	return logdb.Remove(dbName, start, end, nums-minNum)
}
...
func (l LocalLog2) SearchAllRecord(tableName string, primaryFields []string, start, end *time.Time, page, pageSize int) ([]map[string]interface{}, error) {
	...
	res, err := logdb.IteratorByRange(dbName, startstr, endstr)
	if err != nil {
		return nil, err
	}
